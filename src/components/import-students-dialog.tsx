  ...
  useEffect(() => {
    if (isOpen) {
      try {
        ...
        const uniqueSourceClasses = [...new Set(Object.values(profiles).map(p => p.className).filter(Boolean) as string[])].sort();
        setSourceClasses(uniqueSourceClasses);
        
        // Combine predefined and unique classes from profiles for destination
        const uniqueProfileClasses = [...new Set(Object.values(profiles).map(p => p.className).filter(Boolean) as string[])];
        const allPossibleDestClasses = [...new Set([...classLevels, ...uniqueProfileClasses])].sort();
            ...
            <div>
              ...
              <Select value={selectedSourceClass} onValueChange={setSelectedSourceClass}>
                ...
                <SelectContent>
                  ...
                  {sourceClasses.length === 0 && <SelectItem value="-" disabled>No classes with profiles found</SelectItem>}
                </SelectContent>
              </Select>
            </div>
            <div>
                    ...
                    {eligibleStudents.map(student => (
                      <div key={student.studentName} className="flex items-center space-x-3 p-2 rounded hover:bg-muted/30 transition-colors">
                        ...
                        {student.studentPhotoDataUri && (
                          <img src={student.studentPhotoDataUri} alt={student.studentName} className="h-8 w-8 rounded-full object-cover" data-ai-hint="student portrait" />
                        )}

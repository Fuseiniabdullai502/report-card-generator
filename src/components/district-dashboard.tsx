...
export default function DistrictPerformanceDashboard({
  ...
  const renderAiDistrictInsights = () => {
    ...
    return <CardContent className="pt-4 text-sm text-muted-foreground">No AI insights available yet.</CardContent>;
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent id="school-dashboard-dialog-content" className="w-full sm:w-[95vw] max-w-full sm:max-w-6xl h-[calc(100vh-4rem)] flex flex-col">
        ...
        <div data-testid="school-dashboard-inner-scroll-container" className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
          <div className="a4-page-simulation space-y-6 relative">
            ...
            <div className="relative z-10">
              ...
              {isLoadingStats ? (
                <Card><CardContent className="pt-6 flex justify-center"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Loading stats...</CardContent></Card>
              ) : !districtStats ? (
                <Card><CardContent className="pt-6 text-center text-muted-foreground">No reports found for the selected period.</CardContent></Card>
              ) : (
                <>
                  <Card>
                    <CardHeader><CardTitle className="flex items-center"><Sigma className="mr-2" />District Snapshot</CardTitle></CardHeader>
                  ...
                  <Card>
                    <CardHeader><CardTitle className="flex items-center"><Building className="mr-2" />School Performance</CardTitle></CardHeader>
                    <CardContent>
        ...
        <ShadcnDialogFooter className="shrink-0 border-t pt-4 flex-row justify-end dialog-footer-print-hide">
          <Button variant="outline" onClick={handlePrint} disabled={!districtStats} aria-label="Print Dashboard">
            <Printer className="mr-2 h-4 w-4" /> Print Dashboard
          </Button>
          <DialogClose asChild>
            <Button aria-label="Close Dashboard">Close</Button>
          </DialogClose>
        </ShadcnDialogFooter>
      </DialogContent>
    </Dialog>
